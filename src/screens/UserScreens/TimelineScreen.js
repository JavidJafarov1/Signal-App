...
const minutesToTime = totalMinutes => {
  const hours =
    Math.floor(totalMinutes / MINUTES_PER_HOUR) % TOTAL_HOURS_PER_DAY;
...
const getMultiDayEvents = dates => {
  ...
  dates.forEach(date => {
    events.forEach(event => {
      ...
      const eventCrossesIntoCurrentVisibleDate =
        isCrossDay && addDays(eventDate, 1) === date;
      ...
      if (!isCrossDay) {
        ...
        if (eventFallsOnCurrentVisibleDate) {
          allEvents.push({
            ...
            adjustedStart: startMinutes + totalMinutesOffset,
            adjustedEnd:
              TOTAL_HOURS_PER_DAY * MINUTES_PER_HOUR + totalMinutesOffset,
            displayStart: event.start,
            displayEnd: '24:00',
...
const CustomCalendar = () => {
    ...
    if (!isInitialScrollDone && allGeneratedDates.length > 0) {
      ...
      if (initialDateIndex >= 0) {
        ...
        const currentMinuteOffset =
          currentHour * MINUTES_PER_HOUR + currentMinute;

        const scrollPositionWithinDay =
          (currentMinuteOffset / MINUTES_PER_HOUR) * HOUR_WIDTH;
        const totalScrollPosition =
          initialDateIndex * DAY_WIDTH + scrollPositionWithinDay;
        const adjustedScrollPosition = Math.max(
          0,
          totalScrollPosition - (screenWidth - STAGE_WIDTH) / 2,
        );

        setTimeout(() => {
          if (timelineScrollRef.current && eventsScrollRef.current) {
            console.log(
              '--- DEBUG: Initial scroll to X:',
  ...
  const {events: relevantEvents} = useMemo(() => {
    ...
    console.log(
      '--- DEBUG: Processed relevantEvents (count):',
    ...
    visibleDates.forEach((date, index) => {
      ...
      for (let hour = 0; hour < TOTAL_HOURS_PER_DAY; hour++) {
        hours.push({
          ...
          date,
          globalPosition:
            (totalMinutesForTimeline / MINUTES_PER_HOUR) * HOUR_WIDTH,
          key: `hour-${date}-${hour}`,
        });
  ...
  const handleScroll = useCallback(
    event => {
      ...
      if (Math.abs(offsetX - lastScrollX.current) > 5) {
        ...
        const newCurrentDateIndex = Math.floor(
          (offsetX + (screenWidth - STAGE_WIDTH) / 2) / DAY_WIDTH,
        );
        if (
          newCurrentDateIndex >= 0 &&
          newCurrentDateIndex < visibleDates.length
        ) {
          const newSelectedDate = visibleDates[newCurrentDateIndex];
          if (newSelectedDate !== selectedDate) {
            setSelectedDate(newSelectedDate);
            console.log(
              '--- DEBUG: Scroll - New Selected Date:',
              newSelectedDate,
            );
          }
        }
  ...
  const getEventPosition = event => {
    ...
    const duration =
      ((event.adjustedEnd - event.adjustedStart) / MINUTES_PER_HOUR) *
      HOUR_WIDTH;
    return {
        ...
        <View style={{flexDirection: 'row', alignItems: 'center', gap: 10}}>
          ...
          <View>
            ...
            <Text
              style={
                styles.eventTime
              }>{`${event.displayStart}-${event.displayEnd}`}</Text>
          </View>
        </View>
      </TouchableOpacity>
    );
  ...
  const scrollToDate = useCallback(
    (dateToScroll, animated = true) => {
      if (!dateToScroll || !visibleDates.includes(dateToScroll)) {
        console.log(
          '--- DEBUG: scrollToDate aborted - Invalid date:',
          dateToScroll,
        );
        return;
      }
      ...
      const centerAdjustedScrollPosition = Math.max(
        0,
        scrollPosition - (screenWidth - STAGE_WIDTH) / 2,
      );

      if (Math.abs(centerAdjustedScrollPosition - lastScrollX.current) > 5) {
        console.log(
          '--- DEBUG: Scrolling to date:',
          dateToScroll,
          'Position:',
          centerAdjustedScrollPosition,
        );
        timelineScrollRef.current?.scrollTo({
          x: centerAdjustedScrollPosition,
          y: 0,
          animated,
        });
        eventsScrollRef.current?.scrollTo({
      ...
      <ScrollView>
        ...
        <View style={styles.container}>
              ...
              <TouchableOpacity
                ...
                <Text
                  style={[
                    ...
                    !hasPrevious && styles.disabledText,
                  ]}>
                  ‹
                </Text>
              </TouchableOpacity>
          ...
          <View style={styles.timelineHeaderContainer}>
            <View
              style={[styles.stageHeaderPlaceholder, {width: STAGE_WIDTH}]}
            />
            <ScrollView
              ...
              <View
                style={[
                  ...
                  {width: timelineData.totalTimelineWidth},
                ]}>
